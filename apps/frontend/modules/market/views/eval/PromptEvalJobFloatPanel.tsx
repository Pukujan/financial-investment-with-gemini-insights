  ...
  return (
    <div className="rounded-md border border-violet-100 bg-white/90 p-2 space-y-1.5">
      ...
      {(tier.reasoning || tier.error) && (
        <div
          className={`text-[10px] leading-snug max-h-14 overflow-y-auto rounded px-1.5 py-1 ${
...
interface PromptEvalJobFloatPanelProps {
  job: PromptEvalJob;
}

export function PromptEvalJobFloatPanel({ job }: PromptEvalJobFloatPanelProps) {
  const pct =
        ...
        <div className="h-1.5 rounded-full bg-violet-200 overflow-hidden mt-1.5">
          <div
            className={`h-full rounded-full transition-all ${
              job.status === 'failed' ? 'bg-red-500' : job.status === 'completed' ? 'bg-emerald-500' : 'bg-violet-600'
            }`}
            style={{ width: `${pct}%` }}
          />
        </div>
