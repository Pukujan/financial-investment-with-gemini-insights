...
import { CATALYST_TAGS_BY_SYMBOL, SYNTHETIC_DEMO_SOURCES } from '../data/catalystTags';
import { seededIndex, seededUnit, todayDateKey } from '../utils/deterministicSeed';
...
const HEADLINE_TEMPLATES = {
  positive: [
    ...
    'Demo sector pulse highlights constructive setup for {symbol} after trend review',
    'Demo analyst monitor flags steady bid in {symbol} following 30-day chart read',
  ],
  neutral: [
    'Demo market brief tracks mixed signals for {symbol} in latest 30-day trend scan',
    'Demo equity desk sees balanced risk/reward for {company} after trend review',
    'Demo macro desk notes {symbol} trading in line with recent 30-day pattern',
    'Demo earnings watch keeps neutral stance on {symbol} trend context',
  ],
  negative: [
    'Demo traders monitor {symbol} as 30-day trend shows softer momentum',
    'Demo sector pulse flags caution on {company} after recent trend deterioration',
    'Demo analyst monitor highlights weaker volume profile for {symbol}',
    'Demo market brief notes bearish tilt in {symbol} 30-day chart read',
  ],
} as const;

const SUMMARY_TEMPLATES = {
  positive:
    'Synthetic demo interpretation: the Yahoo 30-day trend for {symbol} shows constructive price action with {volumeTrend} volume and {momentum} momentum.',
  neutral:
    'Synthetic demo interpretation: the Yahoo 30-day trend for {symbol} remains mixed with {volumeTrend} volume and {momentum} momentum.',
  negative:
    'Synthetic demo interpretation: the Yahoo 30-day trend for {symbol} shows softer price action with {volumeTrend} volume and {momentum} momentum.',
};
...
export function generateDemoMarketNewsFromTrend(input: {
  ...
  const tags = CATALYST_TAGS_BY_SYMBOL[symbol] ?? ['market trend', 'volume profile', 'price momentum'];
  const vars = {
    symbol,
  ...
  return sentiments.map((sentiment, index) => {
    const seedBase = `${symbol}:${todayDateKey()}:${AGENT_V2_PROMPT_VERSION}:${input.trend.priceChangePercent.toFixed(1)}:${input.trend.volumeTrend}:${input.trend.momentum}:${index}`;
