...
import { useAIInsights } from '../controllers/AIInsightsProvider';

export function AIInsights() {
  const { aiInsights, aiLoading, aiError, aiErrorCode, aiWarnings, refreshInsights } =
    useAIInsights();
  ...
  if (!aiInsights) {
    return (
      <Card>
        <div className="p-6">
          <StatusBanner error={aiError} errorCode={aiErrorCode} warnings={aiWarnings} />
          {!aiError && (
          ...
          {aiError && (
            <div className="text-center">
              <button
                ...
                className="mt-4 px-4 py-2 rounded-lg border border-slate-200 text-slate-700 hover:bg-slate-50 text-sm"
              >
                Retry insights
              </button>
            </div>
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-white/80">
              Advanced analytics and recommendations for smarter decisions
            </p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 bg-gradient-to-br from-blue-50 to-purple-50 rounded-lg border border-blue-200">
            ...
            <div className="flex items-end gap-2 mb-2">
              <span className="text-3xl font-bold text-slate-900">
                {aiInsights.portfolio.diversificationScore}
              </span>
              <span className="text-slate-600 mb-1">/10</span>
            </div>
            <p className="text-sm text-slate-600">{aiInsights.portfolio.diversificationAdvice}</p>
          </div>
          ...
          <div className="p-4 bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg border border-green-200">
            ...
            <div className="flex items-end gap-2 mb-2">
              <span className="text-3xl font-bold text-slate-900">
                {aiInsights.portfolio.growthPotential}
              </span>
              <span className="text-sm text-green-600 mb-1">projected</span>
            </div>
            <p className="text-sm text-slate-600">{aiInsights.portfolio.growthAdvice}</p>
          </div>
        </div>
      </div>
    </div>
