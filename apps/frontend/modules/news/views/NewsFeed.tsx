...
import { useMarketData } from '@/modules/market';
import { filterStatusForDataMode } from '@/modules/market/utils/filterMarketStatus';
import { StatusBanner } from '@/modules/shared/views/StatusBanner';
import { useNewsFeed } from '../controllers/useNewsFeed';

export function NewsFeed() {
  const { news, loading, newsError, newsErrorCode, warnings, dataMode, error } = useMarketData();
  const {
    filteredNews,
    selectedCategory,
    setSelectedCategory,
  ...
  const newsOnlyError = !error ? newsError : null;
  const newsOnlyCode = !error ? newsErrorCode : null;
  const {
    error: displayNewsError,
    errorCode: displayNewsCode,
    warnings: displayWarnings,
  } = filterStatusForDataMode(dataMode, newsOnlyError, newsOnlyCode, warnings);
